...
# Colors
MILD_RED   = (1.0, 0.3, 0.4)
MILD_GREEN = (0.3, 1.0, 0.4)
MILD_BLUE  = (0.3, 0.7, 1.0)

WHITE = (1.0, 1.0, 1.0)
GREY  = (0.25, 0.25, 0.25)
GROUND_LIGHT = (0.6, 0.7, 0.1)
GROUND_DARK  = (0.6, 0.4, 0.1)

FENCE_COLOR  = (0.7, 0.9, 1.0)
UI_TEXT_COLOR = (0.0, 0.0, 0.0)

ACCENT1 = (0.3, 0.55, 0.2)
ACCENT2 = (0.3, 0.55, 0.2)
POST_BASE = (0.95, 0.85, 0.4)
POST_CAP  = (0.95, 0.7, 0.2)
TOWER_BODY= (0.4, 0.4, 0.4)
TOWER_CAP = (1.0, 0.7, 0.9)
...
# Movement, Waves, Collisions, Cheats
...
# Camera, Power-ups, Controls, Loop
def draw_powerup(pu):
...
def ui_overlay(now):
    ...
    mode = "1st Person" if camera_mode == 1 else "3rd Person"
    ...
    draw_text(10, WINDOW_H - 30,
              f"Mode: {mode}   |   Health: {hp_text}   |   Ammo: {COLOR_NAMES[player.ammo_key][0]}")
    draw_text(10, WINDOW_H - 60,
              f"Score: {player.score}  |  Combo: {player.combo}   |   Best: {player.best_combo}   |   Wave: {current_wave}   |   Enemies: {sum(1 for e in enemies if e.alive)}")
    draw_text(10, WINDOW_H - 120, "W/S = Move Forward/Backward   |   A/D = Turn Left/Right")
    draw_text(10, WINDOW_H - 150, "Left Click = Shoot Paintball    |    Right Click = Switch Camera Mode")
    draw_text(10, WINDOW_H - 180, "1 = Red Paintball   |   2 = Green Paintball    |   3 = Blue Paintball")
    draw_text(10, WINDOW_H - 210, "P = Pause   |   R = Reset   |   Esc/X = Quit")
    draw_text(10, WINDOW_H - 240, "H = Infinite Health   |   L = Lava Attack")
    draw_text(10, WINDOW_H - 300, "Cyan Cube = Shield   |   Orange Sphere = Freeze")
...
def keyboardListener(key, x, y):
    ...
    if game_over: return
    if k == b'h': player.cheat_unlimited_hp = not player.cheat_unlimited_hp; return
    if k == b'l': player.aimbot = not player.aimbot; return
...
def main():
    ...
    glutCreateWindow(b"Steve vs Blobs")
